...
class PromptLearner(nn.Module):
    ...
    def forward_test(self, im_features):
        ...
        prompts = []
        for ctx_shifted_i in ctx_shifted:
            ctx_i = ctx_shifted_i.unsqueeze(0).expand(self.n_cls_test, -1, -1)
            pts_i = self.construct_prompts(ctx_i, prefix, suffix)  # (n_cls, n_tkn, ctx_dim)
            prompts.append(pts_i)
        prompts = torch.stack(prompts)
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, classnames_test, clip_model):
        super().__init__()
        self.prompt_learner = PromptLearner(cfg, classnames, classnames_test, clip_model)
    ...
    def forward(self, image, label=None):
        ...
        if self.prompt_learner.training:
            ...
            image_features = self.image_encoder(images.type(self.dtype), masks.type(self.dtype))
            ...
            prompts = self.prompt_learner.forward_test(image_features)
            ...
            for pts_i, imf_i in zip(prompts, image_features):
                ...
                text_features = text_features / text_features.norm(dim=-1, keepdim=True)
                #补充内容
                # text_features = torch.cat([text_features.unsqueeze(1), self.text_features], dim=1)
...
class CoCoOpAlphaCLIP(TrainerX):
