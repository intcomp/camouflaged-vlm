...
class MaskDecoder1Edge(nn.Module):
    def __init__(
        self,
        *,
        transformer_dim: int,
    ...
    def forward(
        ...
        masks, edge, iou_pred = self.predict_masks(
    ...
    def predict_masks(
        ...
        edge = (hyper_in[:, self.num_mask_tokens-1:] @ edge_embedding.view(b, c, h * w)).view(b, -1, h, w)

        edge = self.sigmoid(edge)

        masks = masks * edge + masks
