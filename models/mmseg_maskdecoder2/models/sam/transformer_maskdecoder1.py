...
class TwoWayAttentionBlock(nn.Module):
    def __init__(
        ...
        # self.cross_attn_cond_to_token = Attention(
        #     embedding_dim, num_heads, downsample_rate=attention_downsample_rate
        # )
    ...
    def forward(
        ...
        # add cond_embedding
        q = queries + query_pe
        k = cond_embedding + cond_pe
        attn_out = self.cross_attn_token_to_cond(q=q, k=k, v=cond_embedding)
        queries = queries + attn_out
        ...
        # q = queries + query_pe
        # k = cond_embedding + cond_pe
        # attn_out = self.cross_attn_cond_to_token(q=k, k=q, v=queries)
        # keys = keys + attn_out
