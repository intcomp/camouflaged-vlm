...
@register('sam1')
class SAM(nn.Module):
    def __init__(self, inp_size=None, encoder_mode=None, loss=None, fix=False, use_cache=False):
        super().__init__()
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.fix = fix
        self.use_cache = use_cache  #如果为True，则使用已经保存的sam_feats;反之，还需要加载sam_ImageEncoderViT
        self.clip = ConvNeXtCLIP()
        ...
        # self.visual_feats_to_embs = nn.Linear(256, 768)
    ...
    def set_input(self, input, gt_mask, label, tr_dataset_class_names):
        self.input = input.to(self.device)
        self.gt_mask = gt_mask.to(self.device)
        self.label = label.to(self.device)
        self.class_names = tr_dataset_class_names
    ...
    def infer(self, input, te_dataset_class_names):
        bs = 1
        # Embed prompts
        ...
        # Upscale the masks to the original image resolution
        masks = self.postprocess_masks(low_res_masks, self.inp_size, self.inp_size)
        image_deep = self.get_visual_feats(input, masks)
        self.te_class_names = te_dataset_class_names
        self.test_class_embs = self.clip.get_text_embs_by_template(self.te_class_names)
        normed_class_embs = self.test_class_embs  # Nc,D
        cls_logits = self.map_classifier(low_res_masks, image_deep, normed_class_embs)
        cls_id_per_image = torch.argmax(cls_logits, dim=-1)
        pred_classes = [te_dataset_class_names[i] for i in cls_id_per_image]
        return masks, pred_classes

    def save_feat(self, input, mask_paths):
        self.features = self.image_encoder(input)
        mask_path = mask_paths[0]
        save_path = mask_path.replace('.png', '_vit_h_cache.pth')
        save_path = save_path.replace('mask', 'sam_feats')
        if not os.path.exists(save_path):
            feat = self.features.to(device='cpu')[0]
            torch.save(feat.to(device='cpu'), save_path)
        else:
    ...
    def backward_G(self):
        ...
        self.loss_dict['loss_G'] = self.loss_G.item()
        # self.loss_cls = 0.5 * F.cross_entropy(self.pred_cls, self.label)
        # self.loss_dict['loss_cls'] = self.loss_cls.item()
