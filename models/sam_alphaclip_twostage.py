...
@register('sam_alphaclip_twostage')
class SAM(nn.Module):
    ...
    def set_input(self, input, mask, label_id, image_feat, clip_image, clip_zero_mask):
        self.input = input.to(self.device)
        self.gt_mask = mask.to(self.device)
        self.label_id = label_id.to(self.device)
        self.image_feat = torch.load(image_feat[0]).to(self.device)
    ...
    def forward(self):
        ...
        # image_feat_2, text_feat_2, pred_cls = self.alpha_clip_process(self.clip_image, alpha)
        ...
        # self.pred_mask.append(masks2)

    def infer(self, input, label_id, image_feat_path, clip_image, clip_zero_mask):
        bs = 1
    ...
    def infer_test(self, input, label_id, image_feat_path, clip_image, clip_zero_mask):
        bs = 1
