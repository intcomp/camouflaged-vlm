...
from .mmseg.models.sam import ImageEncoderViT, MaskDecoder, TwoWayTransformer, MaskDecoder1, TwoWayTransformer_maskdecoder1, MaskDecoder1Edge
...
@register('sam_maskdecoder1_edge_iteration')
class SAM(nn.Module):
    def __init__(self, inp_size=None, encoder_mode=None, loss=None):
        ...
        self.mask_decoder = MaskDecoder1Edge(
        ...
        self.mask_downscaling = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv2d(mask_in_chans, encoder_mode['prompt_embed_dim'], kernel_size=1),
        )

    def load_mapleAlphaCLIP(self, maple_clip_model, MaPLeAlphaCLIP_checkpoint):
    ...
    def load_mapleAlphaCLIP_train(self, maple_clip_model):
        self.clip_model = maple_clip_model
        self.clip_model = self.clip_model.float()
        self.clip_model.to(self.device)
        self.clip_model.load_text_features(self.train_text_features, self.test_text_features)

    def set_input(self, input, mask, label_id, clip_image, clip_zero_mask, pred_first_mask=None):
        ...
        self.pred_first_mask = pred_first_mask.to(self.device)
    ...
    def forward(self):
        bs = 1
        # Embed prompts
        not_using_pred_mask = torch.unique(self.clip_zero_mask).numel() == 1
        # print(not_using_pred_mask)
        if not_using_pred_mask:
            dense_embeddings = self.no_mask_embed.weight.reshape(1, -1, 1, 1).expand(
                bs, -1, self.image_embedding_size, self.image_embedding_size
            )
        else:
            dense_embeddings_coarse = self.mask_downscaling(self.pred_first_mask)
            dense_embeddings = dense_embeddings_coarse

        features, interm_embeddings = self.image_encoder(self.input, interm=True)
        image_pe = self.get_dense_pe()

        #第一阶段
        image_feat_1, text_feat_1, pred_1, score = self.maple_alpha_clip_process(self.clip_image, self.clip_zero_mask)
    ...
    def infer(self, input, clip_image, clip_zero_mask, pred_first_mask):
        bs = 1

        # dense_embeddings = self.no_mask_embed.weight.reshape(1, -1, 1, 1).expand(
        #     bs, -1, self.image_embedding_size, self.image_embedding_size
        # )
        dense_embeddings_coarse = self.mask_downscaling(pred_first_mask)
        dense_embeddings = dense_embeddings_coarse
    ...
    def infer_test_encoder(self, input):
        bs = input.shape[0]
        features, interm_embeddings = self.image_encoder(input, interm=True)
        image_pe = self.get_dense_pe()

        return features, interm_embeddings, image_pe

    def infer_test_decoder(self, features, interm_embeddings, image_pe, clip_image, clip_zero_mask, pred_first_mask):
        bs = features.shape[0]
        if pred_first_mask is None:
            dense_embeddings = self.no_mask_embed.weight.reshape(1, -1, 1, 1).expand(
                bs, -1, self.image_embedding_size, self.image_embedding_size
            )
        else:
