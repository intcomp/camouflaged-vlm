...
@register('sam_oridecoder_alphaclip')
class SAM(nn.Module):
    def __init__(self, inp_size=None, encoder_mode=None, loss=None):
        ...
        #
        # self.merge_img = nn.Sequential(
        #     nn.Conv2d(256 * 2, 256, kernel_size=1, padding=0, bias=False),
        #     nn.ReLU(inplace=True),
        # )
        #
        # self.merge_text = nn.Sequential(
        #     nn.Conv2d(256 * 2, 256, kernel_size=1, padding=0, bias=False),
        #     nn.ReLU(inplace=True),
        # )

        self.transformer_decoder = transformer_decoder(50, 256, 512)
    ...
    def alpha_clip_process(self, image, alpha):
        ...
        return image_features, output_text_features, pred_1
    ...
    def forward(self):
        ...
        # image_feat_1 = image_feat_1.unsqueeze(2).unsqueeze(3).repeat(1, 1, features.shape[2], features.shape[3])
        # text_feat_1 = text_feat_1.unsqueeze(2).unsqueeze(3).repeat(1, 1, features.shape[2], features.shape[3])
        #
        # image_feat_1 = self.merge_img(torch.cat([image_feat_1, features], 1))
        # text_feat_1 = self.merge_text(torch.cat([text_feat_1, features], 1))
        protos = self.transformer_decoder(image_feat_1, text_feat_1)
        # sparse_embeddings_1 = torch.cat((image_feat_1, text_feat_1), dim=1)
        sparse_embeddings_1 = protos
        # Predict masks
    ...
    def backward_G(self):
        ...
        self.loss_G = structure_loss(self.pred_mask, self.gt_mask)
        self.loss_G.backward()
